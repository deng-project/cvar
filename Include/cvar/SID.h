...
// file: SID.h - String hashing algorithms
...
namespace CVar {
    ...
    constexpr uint64_t crc64(const char* str) {
        auto prevCrc = crc64<idx - 1>(str);
        return (prevCrc >> 8) ^ crc64_table[(prevCrc ^ str[idx]) & 0xff];
    }

    template<>
    constexpr uint64_t crc64<size_t(SIZE_MAX)>(const char* str) {
    ...
    struct NoHash {
        inline std::size_t operator()(const std::size_t _uKey) const {
            return _uKey;
        }
    };
}
